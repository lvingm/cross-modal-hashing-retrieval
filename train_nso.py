...
def load_data(data_file, mode, batch_size, shuffle):
    data_ = MatDataset2(data_file, mode)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--nl_version', type=str, default='dot', help='version of non-local network')
    ...
    # tensorboard
    writer = SummaryWriter(f'triplet_{args.dataset}_l{args.hash_length}m{args.margin}_k{args.k}')

    # load data
    if args.few_shot == 1:
        tr_data_root = f'./data/{args.dataset}/few_shot/{args.k}shot/NL2ITL.mat'
    else:
        tr_data_root = f'./data/{args.dataset}/NL2ITL.mat'
    data_root = f'./data/{args.dataset}/NL2ITL.mat'
    ...
    ckpt_dir = f'./logs/{args.dataset}/triplet/'
    if args.few_shot == 0:
        I_ckpt_dir = ckpt_dir + f'/IT_NonLocal_{args.nl_version}/I_model/{args.hash_length}bits_lr{args.lr}/'
        T_ckpt_dir = ckpt_dir + f'/IT_NonLocal_{args.nl_version}/T_model/{args.hash_length}bits_lr{args.lr}/'
    ...
    I_model = IFCS(4096, args.hash_length)
    I_model = I_model.to(device)
    if args.dataset == 'pascal':
        T_model = TFCS(300, 2048, args.hash_length)
    elif args.dataset == 'mir-25k':
        T_model = TFCS(1386, 2048, args.hash_length)
    else:
        T_model = TFCS(300, 2048, args.hash_length)
    ...
    if args.load:
        ...
        load_state(I_model, f'/home/disk1/wangshy/FSCM/logs/{args.dataset}/new/IT_fewshot/I_model/{args.hash_length}bits/1m{args.margin}_k{args.k}.pth')
        load_state(T_model, f'/home/disk1/wangshy/FSCM/logs/{args.dataset}/new/IT_fewshot/T_model/{args.hash_length}bits/1m{args.margin}_k{args.k}.pth')
        # calculate original mAP
        I_model.eval()
        T_model.eval()
        tst_IB, tst_TB, tst_labels = IT2binary(test_loader, I_model, T_model, device)
        db_IB, db_TB, db_labels = IT2binary(db_loader, I_model, T_model, device)
        # db_IB, db_TB, db_labels = tst_IB, tst_TB, tst_labels
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=80, gamma=0.5, last_epoch=-1)
    ...
    for epoch in range(args.max_epoch):
        ...
        for batch_idx, (imgs, text, labels) in enumerate(train_loader):
            imgs = imgs.to(device)
