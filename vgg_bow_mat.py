...
class ImgModule(nn.Module):
    def __init__(self):
        ...
        # self.model = torch.load('./vgg19.pth')
...
class IaprDataset(torchdata.Dataset):
    ...
    def __getitem__(self, index):
        # load img
        img_path = './data/iapr-tc12/JPEGImages/' + self.I[index]
...
class MirDataset(torchdata.Dataset):
    ...
    def __getitem__(self, index):
        img_path = './data/mir-25k/JPEGImages/' + self.I[index]
        img = Image.open(img_path).convert('RGB')
        if self.transform is not None:
            img = self.transform(img)
...
class NusDataset(torchdata.Dataset):
    ...
    def __getitem__(self, index):
        img_path = './data/nuswide/JPEGImages/' + self.I[index]
        img = Image.open(img_path).convert('RGB')
        if self.transform is not None:    
            img = self.transform(img)
        label = torch.from_numpy(np.array(self.L[index], dtype='float32'))
...
if __name__ == '__main__':
    ...
    def load_data(IL_file, batch_size, shuffle):
        ...
        return loader_
    tr_ILF = './data/wikipedia_dataset/trainset_txt_img_cat.list'
    te_ILF = './data/wikipedia_dataset/testset_txt_img_cat.list'
    tr_TF = './data/wikipedia_dataset/train_txt.mat'
    te_TF = './data/wikipedia_dataset/test_txt.mat'
    mat_file = './data/wikipedia_dataset/NL2ITL.mat'
    ...
    tr_t = loadmat('./data/wikipedia_dataset/w2v.mat')['tr_W']
    ...
    te_t = loadmat('./data/wikipedia_dataset/w2v.mat')['te_W']
    te_i_ = te_i[idxs]
    te_t_ = te_t[idxs]
    te_l_ = te_l[idxs]
